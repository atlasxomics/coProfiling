...
} else {print("will not change the resolution")
        seuratObj <- RNA }
...
if (n_clusters >= 5){ncol=5
   }else{ncol=n_clusters}
...
dimrd4_r <- list()
Idents(seuratObj) <- seuratObj$seurat_clusters
cells.highlight <- CellsByIdentities(object = seuratObj)[order(names(CellsByIdentities(object = seuratObj)))]
  ...
  for (i in seq_along(samples)) {
    for(j in topGenes){
      top_list[[j]] <- SpatialFeaturePlot(brain_ss[[i]]
                                          ...
                                          , alpha = c(.1, 1)
                                          , pt.size.factor = point_size) +
        theme(legend.position = "top", legend.text=element_text(size=5), legend.title=element_text(size=20))
...
top.features <- head(SpatiallyVariableFeatures_workaround(seuratObj, selection.method = "moransi"), 12)
...
ArrowFiles <- createArrowFiles(
  ...
  minTSS = min_tss, #Dont set this too high because you can always increase later
  minFrags = min_frags,
  addTileMat = TRUE,
...
proj2 <- addGroupCoverages(ArchRProj = proj2
                          , groupBy = "Clusters"
                          ,maxCells = 1500
                          ,force = T)
...
for (run in runs) {
  ...
  ,wait = T)
   ...
   system(paste('rm',paste0(run[1],'_fragments.tsv')),wait = T)    
...
# since rna clustering usually is better than ATAC one, add rna clusters to comapping signac object
...
signac_obj <- RegionStats(  signac_obj
                            , genome = bsgenome)
...
meta.data <- RNA2@meta.data[match(rownames(getCellColData(proj3)),rownames(RNA2@meta.data)),]
...
heatmap <- plotPeak2GeneHeatmap(ArchRProj = proj3
